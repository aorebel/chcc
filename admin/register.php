...
$get_admin_info = "select * from admin where emp_id = ? and email = ?";
$admin_info = $conn->prepare($get_admin_info);
$admin_info->execute(array($id, $email));
		...
		if(!empty($info)){
			...
			if($mail->Send()){
				...
				<div class="w3-modal-content w3-card-4 w3-animate-zoom w3-padding w3-panel w3-red">
				    <h3>Error!</h3>
				    <p>Email cannot be send please use a valid email.</p>
